<script setup>
import { goodGetGoodsDetailAPI } from '@/apis/good.js'
import { ref } from 'vue'
...
const getGoodsDetail = async () => {
  const {
    data: { result }
  } = await goodGetGoodsDetailAPI(route.params.id)
  goods.value = result
}
getGoodsDetail()
onBeforeRouteUpdate(() => {
    ...
    <div class="container">
      <div class="bread-container">
        <el-breadcrumb separator=">">
          ...
          <!-- 错误原因，goods开始是空 =>undefined -->
          <!-- 
            1.使用  ?.  语法连，判断如果有这个东西，才继续，尽量用这个，这样白屏时间没那么长
            2.v-if手动判断
           -->
          <!-- 在后面同样会遇到类似的问题，因为goods对象里面嵌套了多层对象，不像之前写的最对嵌套一层
            当第一层为空，读取就会unfefined，如果再次读取.xxx，就会报无法从undefined里面读取数据
           -->
          <el-breadcrumb-item
        ...
        <div>
          ...
          <div class="goods-footer">
            ...
            <div class="goods-aside">
              <hot-item :hotType="1"></hot-item>
              <hot-item :hotType="2"></hot-item>
            </div>
          </div>
        </div>
      </div>
