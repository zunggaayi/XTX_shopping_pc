<script setup>
import { orderGetUserOrder } from '@/apis/order.js'
import { ref } from 'vue'
...
const getOrderList = async () => {
  ...
  const {
    data: { result }
  } = await orderGetUserOrder(params.value)
  isloading.value = false
  total.value = result.counts
  orderList.value = result.items
  start(result.items[0].countdown)
}
getOrderList()
// el-tabs：tab-change自带的回调函数，值为当前激活的索引
const tabChange = (activeType) => {
  params.value.orderState = activeType
